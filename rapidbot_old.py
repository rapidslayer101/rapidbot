...
import datetime, time, googletrans, google, random, zlib, base64, re, asyncio, discord
...
class Encryption(commands.Cog):
    ...
    async def encrypt(self, ctx):
        ...
        y = 0
        hi = "0"
        ciphertextmangle = ctx.message.content[9:]
                    ...
                    if ciphertextmangle2 == code:
                        ...
                        if total > 2000:
                            with open('config/temp.txt', 'w') as i:
                                i.write(code)
                            embedvar = discord.Embed(title=f"Your encrypted text is over the 2000 char limit as it is {total} chars so has to be sent as a file")
                            embedvar.set_footer(text=f"Requested by {ctx.author}")
                            await ctx.channel.send(embed=embedvar)
                            await ctx.channel.send(file=discord.File('config/temp.txt'))
                        if total < 2000:
                            embedvar = discord.Embed(title=f"Here is your encrypted text ({total} chars):", description=code)
                            embedvar.set_footer(text=f"Requested by {ctx.author}")
                            await ctx.channel.send(embed=embedvar)
                        hi = "1"
                        break
    ...
    async def shorte(self, ctx):
        ...
        embedvar = discord.Embed(title="Here is your encrypted text:", description=num + outputenc)
        embedvar.set_footer(text=f"Took {round(time.time() - started_at,2)} seconds, Requested by {ctx.author}")
        await ctx.channel.send(embed=embedvar)

    @commands.command()
    async def shortd(self, ctx):
        async with ctx.typing():
            started_at = time.time()
            counter = 0
            for i in ctx.message.content[8:]:
                counter = counter + 1
            content = str(ctx.message.content[8:14]).replace("g", "0").replace("e", "1").replace("k", "2").replace("i", "3")\
            ...
            counter = 0
            outputend = ""
            for letter in ctx.message.content[14:]:
                counter = counter + 1
                alphabet = "abcdefghijklmnopqrstuvwxyz"
                partialOne = ""
                partialTwo = ""
        ...
        embedvar = discord.Embed(title="Here is your decrypted text:", description=outputend)
        embedvar.set_footer(text=f"Took {round(time.time() - started_at,2)} seconds, Requested by {ctx.author}")
        await ctx.channel.send(embed=embedvar)
...
class Random(commands.Cog):
    ...
    async def char_count(self, ctx):
        total = 0
        for i in ctx.message.content[12:]:
            total = total + 1
        embedvar = discord.Embed(title=f"Total Number of Characters in this String = {total}")
        embedvar.set_footer(text=f"Requested by {ctx.author}")
        await ctx.channel.send(embed=embedvar)

    @commands.command()
    async def emoji_letters(self, ctx, arg):
        string = ""
        for i in ctx.message.content[15:]:
            if i == " ":
                string = string + ":black_large_square:"
            else:
                string = string + f":regional_indicator_{i}:"
...
async def on_message(message):
    ...
    if not message.channel.id == 745363885271547995:
        ...
        yesid = f"TIME: {rtime} USER ID: {message.author} MESSAGE ID: {message.id}"
        ...
        print(yesid + ' - [ ' + yes + ' ]')
            ...
            #with open('strs/msgstore.txt', 'a+') as f:
                ...
                #yesid = f"TIME: {rtime} USER ID: {message.author} MESSAGE ID: {message.id}"
                #f.write(str(yesid))
                #yes = f"SERVER: {message.guild} CHANNEL: {message.channel} MESSAGE: {msg2}"
                #f.write(str(' - [ ' + yes + ' ]' + "\n"))
                #f.close()

        if not e == "":
            lnkharvest = f"TIME: {rtime}\nSERVER ID: {message.guild.id}\nCHANNEL ID: {message.channel.id}\nUSER ID:\
...
async def on_ready():
    ...
    #Channel = bot.get_channel(746830846790729738)
    #await Channel.purge(limit=5)
    #Text = "React to access server"
...
#bot.run("mfa.uPkjC-EllOjzjbXDajDhq_9Q-IS-QbyjjAiWeePL9wgSUzY8TUSLSTadxcAKyZNx3b5lJ58I2W8uoSKZ7zWT", bot=False) # selfbot
# bot.run("NzQ5OTgzMDcyNzk1MjMwMjc5.X0z6Kg.gl58XLd-ojuQy3zLnFLegTxAf6s") # beta bot
